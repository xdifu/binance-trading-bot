...
GRID_SPACING = float(os.getenv("GRID_SPACING", "1.5"))  # 网格间距(%)，增大可捕获更大波动，减小可提高交易频次
CAPITAL_PER_LEVEL = float(os.getenv("CAPITAL_PER_LEVEL", "6"))  # 每个网格的资金(USDT)，增加可提高利润但需要更多总资金
GRID_RANGE_PERCENT = float(os.getenv("GRID_RANGE_PERCENT", "8"))  # 总网格价格范围(%)，增大可覆盖更大波动，减小则集中在小范围
...
# 非对称网格参数（核心区域优化）
CORE_ZONE_PERCENTAGE = float(os.getenv("CORE_ZONE_PERCENTAGE", "0.5"))  # 核心区域占总范围的比例，增大可集中更多资金在中心价格附近
CORE_CAPITAL_RATIO = float(os.getenv("CORE_CAPITAL_RATIO", "0.7"))  # 核心区域资金比例，增大可增强中心区域交易能力
CORE_GRID_RATIO = float(os.getenv("CORE_GRID_RATIO", "0.6"))  # 核心区域网格点比例，增大可在中心区域创建更多订单

# 订单管理
MAX_ORDER_AGE_HOURS = int(os.getenv("MAX_ORDER_AGE_HOURS", "24"))  # 订单最长存在时间(小时)，减小可更频繁更新长期未成交订单
PRICE_DEVIATION_THRESHOLD = float(os.getenv("PRICE_DEVIATION_THRESHOLD", "0.03"))  # 过期订单距离当前价格阈值(3%)，减小可更激进地更新订单
PROFIT_MARGIN_MULTIPLIER = float(os.getenv("PROFIT_MARGIN_MULTIPLIER", "2.0"))  # 利润与手续费比率要求，增大可提高每笔交易利润要求
BUY_SELL_SPREAD = float(os.getenv("BUY_SELL_SPREAD", "1.5"))  # 买卖差价百分比，增大可提高利润但降低成交概率

# 风险管理设置
TRAILING_STOP_LOSS_PERCENT = float(os.getenv("TRAILING_STOP_LOSS_PERCENT", "4.5"))  # 追踪止损百分比，减小可更早触发止损保护资金
TRAILING_TAKE_PROFIT_PERCENT = float(os.getenv("TRAILING_TAKE_PROFIT_PERCENT", "1.5"))  # 追踪止盈百分比，减小可更早锁定利润
